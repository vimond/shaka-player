...
//TEA/Vimond big int addition.
/**
 * @public {number}
 * @const
 */
shaka.dash.mpd.BIG_NUMBERS_OFFSET_SECONDS = 1451606400; // Jan 1 2016 00:00:00 UTC, e.g. Date.UTC(2016, 0, 1, 0, 0, 0) / 1000
...
shaka.dash.mpd.SegmentTimeline = function() {
  ...
  //TEA/Vimond big int additions.
  this.inheritedTimeScale = 1;
  this.startTimeOffsetBigNumber = null;
};
...
shaka.dash.mpd.SegmentTimeline.prototype.clone = function() {
  ...
  //TEA/Vimond big int additions.
  clone.inheritedTimeScale = this.inheritedTimeScale;
  clone.startTimeOffsetBigNumber = this.startTimeOffsetBigNumber;
...
shaka.dash.mpd.SegmentTimePoint.prototype.clone = function() {
  ...
  //TEA/Vimond big int additions.
  clone.isTooBigNumber = this.isTooBigNumber;
...
shaka.dash.mpd.SegmentTimeline.prototype.parse = function(parent, elem) {
  ...
  //TEA/Vimond big int addition.
  this.inheritedTimeScale = parent.timescale;
  
  this.timePoints = mpd.parseChildren_(this, elem, mpd.SegmentTimePoint);

  //TEA/Vimond big int addition.
  if (this.timePoints[0] && this.timePoints[0].isTooBigNumber) {
    this.startTimeOffsetBigNumber = bigInt(shaka.dash.mpd.BIG_NUMBERS_OFFSET_SECONDS).multiply(this.inheritedTimeScale);
  }
};
...
shaka.dash.mpd.SegmentTimePoint.prototype.parse = function(parent, elem) {
  ...
  // TEA/Vimond big int additions.
  if (this.startTime && !bigInt.isPrecise(this.startTime)) {
    var bigStartTime = bigInt(mpd.parseAttr_(elem, 't', mpd.parseString_));
    if (parent.inheritedTimeScale > 1) {
      //TODO: Compute scaled offset in segment template.
      var scaledOffset = bigInt(shaka.dash.mpd.BIG_NUMBERS_OFFSET_SECONDS).multiply(parent.inheritedTimeScale);
      this.startTime = bigStartTime.subtract(scaledOffset).toJSNumber();
      this.isTooBigNumber = true;
    }
  }
};
