...
/*
//TEA/Vimond troubleshooting for big integer workaround.
function computeEdge(timelineData, availabilityStartTimeSeconds) {
  var timelineLengthSeconds = 
      (timelineData.startOffset + (timelineData.segmentDuration * timelineData.repeat) - timelineData.presentationTimeOffset) / timelineData.timescale;
...
shaka.dash.MpdProcessor.prototype.createManifestInfo_ = function(
  ...
  /*
  //TEA/Vimond troubleshooting for big integer workaround. Please remove.
  var debugInfo = {
    timecodes: [],
    availabilityStartTimeSeconds: mpd.availabilityStartTime,
    availabilityStartTime: new Date(mpd.availabilityStartTime * 1000)
  };
  mpd.periods[0].adaptationSets.forEach(function(s) {
    var data = {};
    if (s.segmentTemplate) {
      data.presentationTimeOffset = s.segmentTemplate.presentationTimeOffset;
      data.timescale = s.segmentTemplate.timescale;
      data.startOffset = s.segmentTemplate.timeline.timePoints[0].startTime;
      data.segmentDuration = s.segmentTemplate.timeline.timePoints[0].duration;
      data.repeat = s.segmentTemplate.timeline.timePoints[0].repeat;
      data.bigStartOffset = s.segmentTemplate.timeline.timePoints[0].bigStartTime;
    }
    var edgeDate = computeEdge(data, debugInfo.availabilityStartTimeSeconds);
    data.edgeIso = edgeDate.toISOString();
    if (data.presentationTimeOffset != 0) {
      throw new Error('Regex fy fy');
    }
    debugInfo.timecodes.push(data);
  });
  
  console.log('Stream summary', debugInfo);
   */  
...
shaka.dash.MpdProcessor.prototype.createStreamInfo_ = function(
  ...
  if (representation.segmentBase) {
    ...
    if (representation.segmentTemplate.timeline && representation.segmentTemplate.timeline.timePoints[0] && representation.segmentTemplate.timeline.timePoints[0].timestampOffset) {
      timestampOffset = representation.segmentTemplate.timeline.timePoints[0].timestampOffset;
    }
  ...
  //TEA/Vimond addition for big integer workaround.
  if (timestampOffset) {
    shaka.log.info('Applying timestamp offset from big integer workaround.', timestampOffset); 
    streamInfo.timestampOffset = -1 * timestampOffset;
  } else if (presentationTimeOffset) {
