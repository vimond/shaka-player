...
shaka.dash.LiveSegmentIndex = function(
  ...
  this.mpd = mpd;
    ...
    /** @public {?number} */
  this.timeShiftBufferDepth = this.mpd.timeShiftBufferDepth;
...
shaka.dash.LiveSegmentIndex.prototype.integrate = function(segmentIndex) {
  ...
  if (temp.timeShiftBufferDepth) {
    shaka.log.debug('timeShiftBufferDepth updated from ' + this.timeShiftBufferDepth + ' to ' + temp.timeShiftBufferDepth + ' during integration from another index.', this.timeShiftBufferDepth, temp.timeShiftBufferDepth);
    this.timeShiftBufferDepth = segmentIndex.timeShiftBufferDepth;
  }
...
shaka.dash.LiveSegmentIndex.prototype.getSeekRangeInternal = function(
  ...
  if (this.timeShiftBufferDepth != null) {
    var seekWindow = currentPresentationTime - this.seekStartTime_;
    shaka.asserts.assert(seekWindow >= 0);
    var seekWindowSurplus = seekWindow - this.timeShiftBufferDepth;
...
shaka.dash.LiveSegmentIndex.prototype.evict_ = function(wallTime) {
  ...
  for (var i = 0; i < this.references.length; ++i) {
    ...
    if ((nextEndTime != null) &&
        (nextEndTime <
         currentPresentationTime - this.timeShiftBufferDepth)) {
      ++remove;
    } else {
      break;
    }
