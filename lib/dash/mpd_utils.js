...
shaka.dash.MpdUtils.createTimeline = function(
  ...
  for (var i = 0; i < timePoints.length; ++i) {
    ...
    // TEA/Vimond addition for big integer timecodes.
    if (timePoints[i].bigStartTime) {
      bigIntOffset = shaka.vimond.Integer.create(timePoints[i].bigStartTime).subtract(tpStart);
    }
    ...
    for (var j = 0; j <= repeat; ++j) {
      ...
      // TEA/Vimond addition for big integer timecodes.
      if (bigIntOffset) {
        result.bigStart = bigIntOffset.add(startTime);
        result.bigEnd = bigIntOffset.add(endTime);
      }
