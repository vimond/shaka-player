...
shaka.util.AjaxRequest.prototype.sendInternal = function() {
  ...
  var url = this.url;
  if (this.estimator && !url.match(/Fragments\(video\=i/)) {
...
shaka.util.AjaxRequest.prototype.onLoad_ = function(event) {
  ...
  if (this.xhr_.status >= 200 && this.xhr_.status <= 299) {
    ...
    this.handleFailure_('HTTP error.');
  }
};
...
shaka.util.AjaxRequest.prototype.onError_ = function(event) {
  ...
  this.handleFailure_('Network failure.');
};
...
shaka.util.AjaxRequest.prototype.onTimeout_ = function(event) {
  ...
  this.handleFailure_('Timeout.');
};


/**
 * Handles a failed network request, whether it be an http/network/timeout error. Retries if configured to do so.
 *
 * @param {string} message The error message.
 *
 * @private
 */
shaka.util.AjaxRequest.prototype.handleFailure_ = function(message) {
  if (this.attempts_ < this.parameters.maxAttempts) {
    ...
    var error = this.createError_(message, 'net');
    this.promise_.reject(error);
    this.destroy_();
  }
};
