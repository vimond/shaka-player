...
shaka.util.AjaxRequest.prototype.send = function() {
  ...
  if (shaka.util.AjaxRequest.enableCacheBusting) {
    if ((this.estimator && !this.estimator.supportsCaching()) ||
      ...
      modifiedUri.getQueryData().add('_', new Date().getTime());
  ...
  if (this.promise_) {
    ...
    var errorMessage = 'Request failed before being sent. Probably blocked by the browser.';
    if (window.location.protocol.indexOf('https') === 0 && this.url && this.url.indexOf && this.url.indexOf('http:') === 0) {
      errorMessage = 'Attempt of accessing insecure content from a secure page blocked by browser.';
    }
...
shaka.util.AjaxRequest.prototype.onLoad_ = function(event) {
  ...
  if (this.estimator) {
    this.estimator.sample(new Date().getTime() - this.startTime_, event.loaded);
  }
  ...
  if (this.xhr_.status >= 200 && this.xhr_.status <= 299) {
    ...
    this.destroy_();
  } else if (this.attempts_ < this.parameters.maxAttempts && this.xhr_.status != 404) {
