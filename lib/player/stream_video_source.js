...
shaka.player.StreamVideoSource.prototype.selectVideoTrack =
    function(id, clearBuffer, opt_abortPendingRequests) {
  return this.selectTrack_('video', id, clearBuffer, opt_abortPendingRequests);
};
...
shaka.player.StreamVideoSource.prototype.selectAudioTrack =
  ...
  return this.selectTrack_('audio', id, clearBuffer, false);
};
...
shaka.player.StreamVideoSource.prototype.selectTextTrack =
    function(id, clearBuffer) {
  return this.selectTrack_('text', id, clearBuffer, false);
};
...
/** @override */
shaka.player.StreamVideoSource.prototype.clearAudioBuffer = function(offset) {
  return this.clearBuffer_('audio', offset);
};


/** @override */
shaka.player.StreamVideoSource.prototype.clearVideoBuffer = function(offset) {
  return this.clearBuffer_('video', offset);
};
 ...
 * @param {boolean=} opt_abortPendingRequests if defined, abort any pending
 *     requests on the underlying stream specified by |type|
 *
 * @return {boolean} True on success.
 * @private
 */
shaka.player.StreamVideoSource.prototype.selectTrack_ =
    function(type, id, clearBuffer, opt_abortPendingRequests) {
    ...
    for (var j = 0; j < streamSetInfo.streamInfos.length; ++j) {
      ...
      if (type != 'text' && !this.canSwitch_) {
        ...
        this.deferredSwitches_[type] = {
          ...
          opt_abortPendingRequests: (tuple != null && tuple.opt_abortPendingRequests) || opt_abortPendingRequests,
        };
        return true;
      }
...
shaka.player.StreamVideoSource.prototype.startStreams_ = function(
  ...
  var setInitialCurrentTime = function(newCurrentTime) {
    this.video.currentTime = newCurrentTime;
    this.video.removeEventListener("loadedmetadata", setInitialCurrentTime);
  }.bind(this, streamStartTime);
...
shaka.player.StreamVideoSource.prototype.processDeferredSwitches_ = function() {
  for (var type in this.deferredSwitches_) {
    ...
    stream.switch(
        ...
        this.manifestInfo.minBufferTime,
        tuple.clearBuffer,
        tuple.opt_abortPendingRequests);
  }
...
/**
 * Clears the |type| buffer forward from the segment boundary nearest the video's
 * current time plus |offset| seconds, to the end.
 *
 * @param {number} offset The number seconds of buffer, in front of video currentTime, to clear.
 *
 * @private
 */
shaka.player.StreamVideoSource.prototype.clearBuffer_ = function(type, offset) {
  if (!this.streamsByType_[type]) {
    shaka.log.warning(
        'Cannot clear', type, 'buffer',
        'because there are no', type, 'streams yet.');
    return;
  }

  this.streamsByType_[type].clearBuffer(offset);
};
