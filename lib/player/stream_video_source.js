...
shaka.player.StreamVideoSource.prototype.destroy = function() {
  ...
  this.attachPromise_ && this.attachPromise_.destroy();
  this.proceedPromise_ && this.proceedPromise_.destroy();
  ...
  this.streamsByType_ && shaka.util.MapUtils.values(this.streamsByType_).forEach(
      function(stream) {
        stream.destroy();
      });
  ...
  this.abrManager_ && this.abrManager_.destroy();
    ...
    p.then(shaka.util.TypedBind(this,
        ...
        function(error) {
          ...
          if (error.type != 'aborted') {
            if (error.status == 404 && this.manifestInfo.live) {
              this.isEnded_ = true;
              this.endOfStream_();
