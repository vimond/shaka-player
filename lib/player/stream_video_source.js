...
shaka.player.StreamVideoSource = function(manifestInfo, estimator, abrManager) {
  ...
  /* TEA/Vimond addition: Making isLive() reflect the updated live state of a stream, not only the initial mode */
  /** @protected {boolean|null} */
  this.updatedLive_ = null;
...
shaka.player.StreamVideoSource.prototype.configure = function(config) {
  ...
  if (config['smallGapsTolerance'] != null) {
	  this.streamConfig_['smallGapsTolerance'] =
		  config['smallGapsTolerance'];
  }
  ...
  shaka.player.StreamVideoSource.prototype.onUpdateManifest_ =
    ...
    p.then(shaka.util.TypedBind(this,
        ...
        function(newManifestInfo) {
          ...
          /* TEA/Vimond addition: Making isLive() reflect the updated live state of a stream, not only the initial mode */
          this.updatedLive_ = newManifestInfo.live;
...
shaka.player.StreamVideoSource.prototype.removeStream_ = function(streamInfo) {
  ...
  if (stream && (stream.getStreamInfo() == streamInfo)) {
    ...
    if (this.deferredSwitches_[type] && this.deferredSwitches_[type].streamInfo == streamInfo) {
...
shaka.player.StreamVideoSource.prototype.isLive = function() {
  /* TEA/Vimond addition: Reflecting the updated live state of a stream, not only the initial mode */
  return this.updatedLive_ == null ? this.manifestInfo.live : this.updatedLive_; // return this.manifestInfo ? this.manifestInfo.live : false;
};
...
shaka.player.StreamVideoSource.prototype.onMediaSourceOpen_ = function(event) {
  ...
  try {
    var r = this.createAndStartStreams_();
    r.then(shaka.util.TypedBind(this,
        function () {
    ...
    if (r) {
      var er = new Error('createAndStartStreams_() failed: ' + e.message);
      if (this.attachPromise_) {
        this.attachPromise_.reject(er);
      } else {
        throw er;
      }
    } else {
      var err = new Error('createAndStartStreams_ undefined.');
      if (this.attachPromise_) {
        this.attachPromise_.reject(err);
      } else {
        throw err;
      }
    }
...
shaka.player.StreamVideoSource.prototype.startStreams_ = function(
  ...
  // TEA/Vimond addition. If the start time is earlier than the start limit, we assume that the start time is relative to the start limit.
  if (this.playbackStartTime_ && this.playbackStartTime_ <= streamLimits.end &&
    this.playbackStartTime_ >= streamLimits.start) {
    streamStartTime = this.playbackStartTime_;
  } else if (this.playbackStartTime_ && streamLimits.start > this.playbackStartTime_) {
    streamStartTime = this.playbackStartTime_ + streamLimits.start;
...
shaka.player.StreamVideoSource.prototype.processDeferredSwitches_ = function() {
  for (var type in this.deferredSwitches_) {
    try {
      ...
      if (tuple) {
        throw new Error('Tuple undefined for ' + type);
      } else {
        throw new Error('Deferred switches processing failed. ' + e.message);
      }
    }
  }
