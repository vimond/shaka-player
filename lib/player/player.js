...
goog.require('shaka.vimond.dash.SmallGapsTolerancePolicy');
 ...
 */
goog.define('GIT_VERSION', 'v1.6.10-vimond');
...
shaka.player.Player.prototype.destroy = function() {
  var ul = this.unload();
  if (ul) {
    return ul.then(shaka.util.TypedBind(this, function () {
...
shaka.player.Player.prototype.load = function(videoSource) {
  ...
  return loaded.then(shaka.util.TypedBind(this,
      function() {
        ...
        if (this.liveShutdownOnError_) {
          this.eventManager_.listen(this.videoSource_, 'error', this.onSourceError_.bind(this));
        }
...
shaka.player.Player.prototype.onPlaying_ = function(event) {
  ...
  if (!this.rewindTimer_ && this.playbackRate_ < 0) {
    ...
    this.onRewindTimer_(
        this.video_.currentTime, new Date().getTime(), this.playbackRate_);
  }
  if (this.buffering_) {
    this.endBufferingState_();
  }
...
shaka.player.Player.prototype.setPlaybackRate = function(rate) {
  ...
  // Only rewind when not paused.
    ...
    this.video_.playbackRate = 0;
    this.onRewindTimer_(this.video_.currentTime, new Date().getTime(), rate);
  }
...
shaka.player.Player.prototype.configure = function(config) {
  ...
  var shutdownOnLiveError = MapUtils.getBoolean(config,
      'enableShutdownOnLiveError');
  if (shutdownOnLiveError != null) {
    this.liveShutdownOnError_ = shutdownOnLiveError;
  }

  var smallGapsConfig = MapUtils.getAsInstanceType(
	  config, 'smallGapsTolerance', shaka.vimond.dash.SmallGapsTolerancePolicy);
  if (smallGapsConfig != null) {
      this.videoSourceConfig_['smallGapsTolerance'] = smallGapsConfig.clone();
  }
...
shaka.player.Player.prototype.onRewindTimer_ =
  ...
  var offset = ((new Date().getTime() - startWallTime) / 1000) * rate;
