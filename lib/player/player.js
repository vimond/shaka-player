...
shaka.player.Player.prototype.setVideoEventListeners_ = function() {
  ...
  this.eventManager_.listen(this.video_, 'seeked', this.onSeeked_.bind(this));
};
...
shaka.player.Player.prototype.onSessionMessage_ = function(event) {
  ...
  this.requestLicense_(event.target, this.drmScheme_.licenseServerUrl,
                       ...
                       this.drmScheme_.licensePostProcessor, this.drmScheme_.keySystem);
};
 ...
 *
 * @param {!ArrayBuffer} body of license request
 * @param {!Object.<string, string>=} headers extra headers.
 * @return {string}
 *
 * @private
 */
shaka.player.Player.prototype.playReadyLicensePreProcessor_ = function(body, headers){
  /*
  ...
  var licenseBodyXml = String.fromCharCode.apply(null, new Uint16Array(body['buffer']));
  ...
  return window.atob(licenseBodyXmlDom.getElementsByTagName("Challenge")[0].childNodes[0].nodeValue);
}
 ...
 * Required for IE Edge as edge doesn't fire onPlaying event when starting a video so the watchdog timer doesn't get initialised
 * @param event
 * @private
 */
shaka.player.Player.prototype.onSeeked_ = function(event) {
  shaka.log.debug('onSeeked', event);
...
/**
 * Starts the watchdog timer if it doesn't exist
 * @private
 */
...
shaka.player.Player.prototype.onWatchdogTimer_ = function() {
  ...
  if (!this.buffering_) {
    ...
    if (fudgedBufferEnd < this.video_.duration && underflow > shaka.player.Player.UNDERFLOW_THRESHOLD_) {
 ...
 * This value has been changed to the same value dash.js as the buffering event was not getting
 * fired for IE Edge.
 * @private {number}
 * @const
 */
