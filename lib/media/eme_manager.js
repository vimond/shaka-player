...
shaka.media.EmeManager = function(player, video, videoSource) {
  ...
  /** @private {shaka.util.EventManager.ListenerType} */
  this.boundOnPrematureEncrypted_ = shaka.util.TypedBind(this, this.onPrematureEncrypted_);
  
  /** @private {Array.<MediaEncryptedEvent>} */
  this.prematureEncryptedEvents_ = [];

  this.eventManager_.listen(
      this.video_,
      'encrypted', 
      this.boundOnPrematureEncrypted_);
};
  ...
  return this.video_.setMediaKeys(this.mediaKeys_).then(
    ...
    if (this.videoSource_.getSessionIds().length > 0) {
      ...
      if (this.drmInfo_.initDatas.length == 0) {
        ...
        // TEA/Vimond addition. This event binding happened too late in Edge, and the event we were waiting for was often missed.
        if (this.drmInfo_.keySystem == 'com.microsoft.playready' && this.prematureEncryptedEvents_.length > 0) { // Edge only at this time.
          window.setTimeout(function() { // Do it right after this block. 
            var evt;
            while (evt = this.prematureEncryptedEvents_.shift()) {
              shaka.log.debug('Applying prematurely received encrypted event.');
              this.onEncrypted_(evt);
            }
          }.bind(this),10);
        }
        this.eventManager_.unlisten( // Stop the premature encrypted event listener.
            this.video_,
            'encrypted');
...
/**
 * Premateure EME 'encrypted' event handler. Needed for Edge. With the normal event listener attachment, the encrypted event is often missed, and license acuisition is never performed.
 *
 * @param {!Event} event The EME 'encrypted' event.
 * @private
 */
shaka.media.EmeManager.prototype.onPrematureEncrypted_ = function(event) {
  //shaka.log.debug('Received premature encrypted event.');
  this.prematureEncryptedEvents_.push(event);
};
...
shaka.media.EmeManager.prototype.createSession_ = function() {
  ...
  if (this.videoSource_.isOffline()) {
    ...
    session = this.mediaKeys_.createSession('temporary');
  }
