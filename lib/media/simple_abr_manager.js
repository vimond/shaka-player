 ...
 * @const {number}
 */
shaka.media.SimpleAbrManager.MIN_SWITCH_INTERVAL_ = 4000;
...
shaka.media.SimpleAbrManager.prototype.start = function() {
  ...
  this.nextAdaptationTime_ =
      new Date().getTime() + shaka.media.SimpleAbrManager.FIRST_SWITCH_INTERVAL_;
  //shaka.log.info('Checking adaptation after ' + Math.round(shaka.media.SimpleAbrManager.FIRST_SWITCH_INTERVAL_/1000) + ' seconds');
...
shaka.media.SimpleAbrManager.prototype.onBandwidth_ = function(event) {
  if (new Date().getTime() < this.nextAdaptationTime_) {
    return;
  }
...
shaka.media.SimpleAbrManager.prototype.chooseNewTrack_ = function(event) {
  ...
  if (chosen) {
    if (chosen.active) {
      ...
      this.nextAdaptationTime_ =
          new Date().getTime() + SimpleAbrManager.MIN_EVAL_INTERVAL_;
      //shaka.log.info('Checking adaptation after ' + Math.round(SimpleAbrManager.MIN_EVAL_INTERVAL_/1000) + ' seconds');
      return;
    }
  ...
  this.nextAdaptationTime_ = Number.POSITIVE_INFINITY;
  //shaka.log.info('Not checking adaptation again');
};
...
shaka.media.SimpleAbrManager.prototype.onAdaptation_ = function(event) {
  ...
  if (this.nextAdaptationTime_ == Number.POSITIVE_INFINITY) {
    ...
    this.nextAdaptationTime_ =
        new Date().getTime() + shaka.media.SimpleAbrManager.MIN_SWITCH_INTERVAL_;
    //shaka.log.info('Checking adaptation after ' + Math.round(shaka.media.SimpleAbrManager.MIN_SWITCH_INTERVAL_/1000) + ' seconds');
  }
};
...
shaka.media.SimpleAbrManager.prototype.chooseVideoTrack_ = function() {
  ...
  //shaka.log.info('Bandwidth reported', (bandwidth / 1000).toFixed(2));
