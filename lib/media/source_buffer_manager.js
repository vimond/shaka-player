...
shaka.media.SourceBufferManager.prototype.clearAfter = function(timestamp) {
  ...
  shaka.asserts.assert(!this.task_);
  if (this.task_) {
    var error = new Error('Cannot clearAfter: previous operation not complete.');
    error.type = 'stream';
    return Promise.reject(error);
  }
...
shaka.media.SourceBufferManager.prototype.clearAfter_ = function(timestamp) {
  ...
  var segmentIndex = shaka.media.SegmentReference.find(this.inserted_, timestamp)

  // If no segment found, or it's the last one, bail out gracefully.
  if (segmentIndex == -1 || segmentIndex == this.inserted_.length - 1) {
  ...
  try {
    // This will trigger an 'updateend' event.
    this.sourceBuffer_.remove(this.inserted_[segmentIndex + 1].startTime, Number.POSITIVE_INFINITY);
  } catch (exception) {
    shaka.log.debug('Failed to clear buffer:', exception);
    return Promise.reject(exception);
  }

  this.inserted_ = this.inserted_.slice(0, segmentIndex + 1);
