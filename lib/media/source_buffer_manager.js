...
shaka.media.SourceBufferManager.prototype.fetchFromMultipleUrls_ = function() {
  ...
  var requestSegment = function(reference) {
    ...
    this.request_.parameters.maxAttempts = 3;
    this.request_.estimator = this.estimator_;
 ...
 * representations. However the buffer segments between startIndex and endIndex
 * are being retained, so they should be re-inserted
 */
shaka.media.SourceBufferManager.prototype.resetWithRetention = function(startIndex, endIndex) {
  // Ensure that the segment between the supplied startIndex and endIndex are actually inserted
  var index;
  for (index = startIndex; index <= endIndex; index++) {
    if (!this.inserted_[index]) {
      endIndex = index - 1; // The previous index was the final one that was inserted
      break;
    }
  }
