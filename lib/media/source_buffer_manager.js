...
shaka.media.SourceBufferManager = function(
  ...
  /** @private {boolean} */
  this.shouldReportSmallGaps_ = true;
  ...
  this.task_.append(
      function() {
        ...
        if (computeTimestampCorrection) {
          ...
          // timestampCorrection = this.subtractWithoutFloatError_(actualTimestamp, expectedTimestamp);
        }
 ...
 * 
 * @param {?number} smallGapsLimit If specified, indicates the max length of gaps to be allowed, and still consider several ranges as one.
 * @return {boolean}
 */
shaka.media.SourceBufferManager.prototype.detectMultipleBufferedRanges =
    function(smallGapsLimit) {
  var buffered = this.sourceBuffer_.buffered;
  if (buffered.length > 1) {
    if (smallGapsLimit) {
		for (var i = 1; i < buffered.length; i++) {
			var gapLength = buffered.start(i) - buffered.end(i - 1);
			//shaka.log.warning(this.logPrefix_(), 'Range gap in seconds:', gapLength);
			if (gapLength > smallGapsLimit) {
				shaka.log.error(
					this.logPrefix_(),
					'multiple buffered ranges with significant gaps detected:',
					'Either the content has gaps > ' + smallGapsLimit + ' seconds in it,',
					'the content\'s segments are not aligned across bitrates,',
					'or the browser has evicted the middle of the buffer.', gapLength, buffered.start(i), buffered.end(i - 1));
				return true;
			} else if (this.shouldReportSmallGaps_) {
			  shaka.log.info('Small gap detected. Start/end/length:', buffered.end(i - 1), buffered.start(i), gapLength);
            }
		}
		this.shouldReportSmallGaps_ = false;
		return false;
	} else {
		shaka.log.error(
			this.logPrefix_(),
			...
			'the content\'s segments are not aligned across bitrates,',
			'or the browser has evicted the middle of the buffer.', buffered);
		return true;
    }
  } else {
    return false;
...
/*
-XX This is a camouflaged JSDoc comment, avoiding attention from Google Closure Compiler.
XX* Fixes Javascript float precision errors spoiling the timestamp correction.
XX* @param {number} originalNumber
XX* @param {number} number
XX* @returns {number}
XX* @private
XX*-
shaka.media.SourceBufferManager.prototype.subtractWithoutFloatError_ = function(numberA, numberB) {
  try {
    var a = numberA.toString(), 
        b = numberB.toString();
    if (a.indexOf('e') < 0 && a.indexOf('e') < 0) {
      var splitA = a.split('.'), 
          splitB = b.split('.');
      var digitsA = splitA.length > 1 ? splitA[1].length : 0, 
          digitsB = splitB.length > 1 ? splitB[1].length : 0;
      var digits = Math.max(digitsA, digitsB);
      if (digits > 0) {
        var number = numberA - numberB;
        var re = new RegExp("(-?\\d+\\.\\d{" + digits + "})(\\d)"),
            m = number.toString().match(re);
        var result = m ? parseFloat(m[1]) : number.valueOf();
        if (result.toString().length < number.toString().length) {
          shaka.log.debug(this.logPrefix_(),' Corrected timestamp floating point computation error.', number, result);
        }
        return result;
      }
    }
...
shaka.media.SourceBufferManager.prototype.startTask_ = function() {
  shaka.asserts.assert(this.task_);
  this.task_.start();
  var thenable = this.task_.getPromise();
  if (!thenable) {
    throw new Error('Task didn\'t return a promise.');
  }
  return thenable.then(shaka.util.TypedBind(this,
      function() {
        this.task_ = null;
      })
