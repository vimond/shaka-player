...
shaka.media.Stream.prototype.configure = function(config) {
  ...
  if (config['smallGapsTolerance'] != null) {
    var smallGapsTolerance = config['smallGapsTolerance'];
    if (smallGapsTolerance.maxGapLength && smallGapsTolerance.userAgentMatch && navigator.userAgent.match(smallGapsTolerance.userAgentMatch)) {
		this.smallGapsLimit_ = smallGapsTolerance.maxGapLength;
		shaka.log.info(this.logPrefix_(), 'Applying small gap tolerance according to user agent match.', this.smallGapsLimit_);
    }
  }
};
...
shaka.media.Stream.prototype.started = function(proceed) {
  if (!this.proceeded_) {
    if (!proceed) {
      throw new Error('Stream started method failing on undefined proceed argument.');
    }
    proceed.then(
        function() {
...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  if (this.proceeded_ &&
      !this.ended_ &&
      this.sbm_.detectMultipleBufferedRanges(this.smallGapsLimit_)) {
  ...
  if (!fetch) {
    throw new Error('SourceBufferManager fetch is undefined.');
  }
...
shaka.media.Stream.prototype.getNext_ = function(
    ...
    if (last != null) {
        ...
        var next = segmentIndex.find(currentTime);
        if (next) {
            return next;
        } else {
            if (currentTime < segmentIndex.first().startTime) {
                shaka.log.debug(this.logPrefix_(), 'trying to seek before the earliest segment start time.', currentTime);
              return segmentIndex.first();
            } else {
                ...
                return segmentIndex.length() ? segmentIndex.last() : null;
            }
        }
    }
};
