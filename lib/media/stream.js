  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      function() {
        ...
        if (immediate) {
          ...
          var currentStreamInfo = this.getStreamInfo();
          var switchUp = currentStreamInfo.bandwidth < streamInfo.bandwidth;

          shaka.asserts.assert(estimatedFetchLatency < bufferedAhead,
              'Not enough data buffered to switch smoothly!');

          if (switchUp) {
            // switching up, so switch as soon as possible to display the best quality.
            switchTime = this.video_.currentTime + Math.min(estimatedFetchLatency, bufferedAhead - 1);
            // reset only on switching up, else keep retain as much of the buffer as possible
            this.sbm_.reset();
          } else {
            switchTime = this.video_.currentTime + (
                    bufferedAhead > estimatedFetchLatency ?
                      bufferedAhead - estimatedFetchLatency :
                      bufferedAhead
                );
          }
