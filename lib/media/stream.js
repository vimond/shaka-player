 ...
 * @param {?number} playbackStartTime The time (in seconds) you want to
 ...
 */
shaka.media.Stream =
    function(parent, video, mediaSource, sourceBuffer, estimator, playbackStartTime) {
  ...
  /** @private {?number} */
  this.playbackStartTime_ = playbackStartTime;
...
shaka.media.Stream.prototype.switch = function(
    streamInfo, minBufferTime, clearBuffer, opt_abortPendingRequests) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      ...
      function(results) {
        ...
        if (opt_abortPendingRequests) {
          // Do not clear buffer, we just want to abort requests in the SBM,
          // and allow resync to handle the restarting of the update loop
          this.resync_(false);
        }
 ...
 * @param {number=} opt_offset if defined, only clear buffer after |opt_offset| seconds in front of video currentTime
 * @param {boolean=} opt_avoidNudge if defined and truthy, don't apply the nudge
 * @private
 */
shaka.media.Stream.prototype.resync_ = function(forceClear, opt_offset, opt_avoidNudge) {
  ...
  this.sbm_.abort().then(shaka.util.TypedBind(this,
      function() {
        ...
        if (forceClear ||
          ...
          if (opt_offset) {
            return this.sbm_.clearAfter(currentTime + opt_offset);
          }
          else {
...
/** @override */
shaka.media.Stream.prototype.clearBuffer = function(offset) {
  // Only trigger the resync (with offsets to clear the buffer after currentTime + offset), if
  // currentTime + offset has actually been inserted. E.g. if the buffered ahead amount is
  // actually smaller than currentVideo + offset, no action needs to be taken.
  if (this.sbm_.isInserted(this.video_.currentTime + offset)){
    this.resync_(true /*forceClear*/, offset /*opt_offset*/, true /*opt_avoidNudge*/);
  }
};
...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  // Edge specific nudge, where playback pipeline is getting "stuck"; when
  // setting an initial currentTime > 0 and the buffer eventually fills out
  // around it, even though the player is reporting that it's playing, it's not
  // and calling pause() then play() resolves it. Stangely, when in this "stuck"
  // state, minimising then restoring the windows resumes playback. The existing
  // currentTime nudge doesn't work in Edge.
  var needsPausePlayNudge = false;

  // If currentTime is 0 but this.playbackStartTime_ is > 0, then playback is
  // intended to start mid-stream (at this.playbackStartTime_), so we can base
  // all buffering decisions on this value in this scenario.
  if (currentTime == 0 && this.playbackStartTime_ && this.playbackStartTime_ > 0) {
    currentTime = this.playbackStartTime_;

    this.needsNudge_ = true;
    needsPausePlayNudge = true;
  }
  ...
  fetch.then(shaka.util.TypedBind(this,
      ...
      function(timestampCorrection) {
        ...
        if (this.needsNudge_ && this.sbm_.bufferedAheadOf(currentTime) > 0) {
          ...
          if (needsPausePlayNudge) {
            needsPausePlayNudge = false;
            this.video_.pause();
            this.video_.play();
          }
        }
