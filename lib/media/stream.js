...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  // If currentTime 0.5s or less but this.playbackStartTime_ is > 0.5, then playback is
  // intended to start mid-stream (at this.playbackStartTime_), so we can base
  // all buffering decisions on this value in this scenario. NOTE: We use 0.5s to
  // allow for some "nudging" in the other streams, as the video element is global, the
  // currentTime may not be exactly zero due to nudges. But it's effectively zero.
  if (currentTime <= 0.5 && this.playbackStartTime_ && this.playbackStartTime_ > 0.5) {
