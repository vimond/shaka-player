...
shaka.media.ManifestUpdater.prototype.destroy = function() {
  ...
  var timelines = this.newManifestInfo_.destroy();
 ...
 */
shaka.media.ManifestUpdater.prototype.update = function(currentManifestInfo, previousTimelines) {
  // Alias.
  ...
  var promise1 = ManifestUpdater.createSegmentIndexes_(currentManifestInfo, previousTimelines);
  var promise2 = ManifestUpdater.createSegmentIndexes_(this.newManifestInfo_, previousTimelines);
...
shaka.media.ManifestUpdater.createSegmentIndexes_ = function(
    manifestInfo, previousReferences) {
  var gather = function(all, part) { return all.concat(part); };
  var streamInfos = manifestInfo.periodInfos
      .map(function(periodInfo, index) { return periodInfo.streamSetInfos; })
      .reduce(gather, [])
  ...
  var async = streamInfos.map(
      function(streamInfo) {
        //TEA/Vimond addition for CPU offload in dynamic timelines.
        var prevRefs = streamInfo.id && previousReferences && previousReferences[streamInfo.id];
        return streamInfo.segmentIndexSource.create(prevRefs);
      });
