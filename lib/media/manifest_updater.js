...
shaka.media.ManifestUpdater.mergeStreamInfos_ = function(
  ...
  for (var i = 0; i < keys.length; ++i) {
    ...
    var cloned = newStreamInfos[0].clone();
    newStreamInfos[0].segmentIndexSource = null;
    newStreamInfos[0].segmentInitSource = null;
    // Making sure the updater.destroy() method not destroys the added segment*Sources.
      
    currentStreamSetInfo.streamInfos.push(cloned);
    shaka.log.info('Added Stream ' + id + '. Beware of memory leaks.');
  }
};
